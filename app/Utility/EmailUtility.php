...
class EmailUtility
{
    ...
    public static function order_email($order, $status)
    {
        ...
        // Check if it's a split payment order and status is 'paid' or 'partial_paid'
        $isSplitPayment = isset($order->is_split_payment) && $order->is_split_payment;
        
        foreach($users as $user){ 
            
            // Determine email identifier based on user type and payment status
            $emailIdentifier = 'order_'.$status.'_email_to_'.$user->user_type;
            
            // For split payment orders, use specific templates
            if ($isSplitPayment) {
                if ($status == 'partial_paid') {
                    // Email for advance payment (30%)
                    $emailIdentifier = 'order_advance_paid_email_to_'.$user->user_type;
                } elseif ($status == 'paid' && $order->advance_payment_status == 'paid') {
                    // Email for complete payment (both advance + COD)
                    $emailIdentifier = 'order_split_payment_complete_email_to_'.$user->user_type;
                }
            }
            ...
            if($emailTemplate == null) {
                $emailTemplate = EmailTemplate::whereIdentifier('order_'.$status.'_email_to_'.$user->user_type)->first();
            }
            ...
            if($emailTemplate != null && $emailTemplate->status == 1){
                ...
                // Add split payment specific placeholders
                if ($isSplitPayment) {
                    $advanceAmount = isset($order->advance_amount) ? $order->advance_amount : ($order->grand_total * 0.30);
                    $codAmount = isset($order->cod_amount) ? $order->cod_amount : ($order->grand_total * 0.70);
                    
                    $emailBody = str_replace('[[advance_amount]]', single_price($advanceAmount), $emailBody);
                    $emailBody = str_replace('[[cod_amount]]', single_price($codAmount), $emailBody);
                    $emailBody = str_replace('[[payment_method_advance]]', isset($order->advance_payment_details) ? 'Online (PayU)' : 'Online', $emailBody);
                    $emailBody = str_replace('[[payment_method_cod]]', 'Cash on Delivery', $emailBody);
                    
                    if ($status == 'partial_paid') {
                        $emailBody = str_replace('[[order_amount]]', single_price($advanceAmount), $emailBody);
                        $emailBody = str_replace('[[remaining_amount]]', single_price($codAmount), $emailBody);
                    } else {
